...
Use profile `Win7SP0x86` or any profile you like and list all running process. Use `pstree`:
...
Show further info of this process with PID `2772`. Use `cmdline`
...
Or using this command. `-n` for excluding the "trailing newline" and remember a special case in **Unix system**, `echo "\U"` will give nothing (because `\u` and `\U` is a unicode specification. Thus, standalone `\U` will be deleted) use `echo -E "\\U"` to correctly execute. 
